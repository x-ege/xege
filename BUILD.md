...
在 Linux/macOS 上，EGE 目前有两条构建路径：

- **默认（兼容）模式：交叉编译到 Windows `.exe`**
  - 需要安装 [mingw-w64](https://www.mingw-w64.org/) 工具链；`CMakeLists.txt` 会在 Linux/macOS 下自动改用 mingw-w64。
  - 运行交叉编译出来的 `.exe` 需要使用 [wine](https://www.winehq.org/) 模拟层。
  - 为方便起见，可以添加 `-static` 配置编译，免去 wine 环境中 DLL 配置过程（仓库默认已在 Linux 环境下开启相关配置）。

- **OpenGL 模式：原生 (native) + GLFW/OpenGL（真跨平台）**
  - 通过 CMake 选项 `-DEGE_BUILD_OPENGL=ON` 开启。
  - 在 Linux/macOS 上将强制走原生编译（不再走 mingw-w64 / wine 路径），运行产物为原生可执行文件（通常无 `.exe` 后缀）。
...
## Linux/macOS：启用 OpenGL 原生模式（推荐）

你可以通过 CMake 选项 `-DEGE_BUILD_OPENGL=ON` 直接构建原生版本。示例：

- 直接使用 CMake（示例为 Debug）：
  1. 创建单独的构建目录（避免与默认交叉编译目录混用）
  2. 配置并编译
