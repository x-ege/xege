...
最终统一分支采用两条方案的优点并形成一个 merge 历史：以持久 CPU Bitmap 的裸指针兼容语义
作为正确性主线，保留 GPU 脏矩形和精确区域更新，并吸收性能诊断与多进程统计工具。依赖调用方
额外标记可写缓冲区的方案没有保留，因为它无法保证旧程序通过保留指针再次写入仍被观察到。
...
- 机器相关时间不作为 CTest 门禁阈值。原始日志、逐轮数据、汇总和环境快照位于
  `build/pixel-performance-results/hybrid-final/`；这是记录提交 `0109747` 的合并前历史数据。
