    ...
    bool initNet(int w, int h, PIMAGE inputTexture, PIMAGE outputTarget)
    {
        ...
        for (int i = 0; i != h; ++i) {
            ...
            for (int j = 0; j != w; ++j) {
                ...
                m_vec[0][index]    = WavePoint(widthJ, heightI, widthJ, heightI);
                m_vec[1][index]    = WavePoint(widthJ, heightI, widthJ, heightI);
...
private:
    std::vector<WavePoint> m_vec[2];
    std::vector<WavePoint> m_pointCache;
    int                m_index;
