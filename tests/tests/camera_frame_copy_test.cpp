#include "../../src/camera_frame_copy.h"
...
bool bytesEqual(const unsigned char* lhs, const unsigned char* rhs, std::size_t count)
{
    return std::memcmp(lhs, rhs, count) == 0;
}

bool bytesAre(const unsigned char* bytes, std::size_t count, unsigned char value)
{
...
int main()
{
    ...
    const BgraFrameView padded = {paddedSource.data(), paddedSource.size(), 3, 2, 16};
    ...
    expect(bytesEqual(paddedDestination.data() + rowBytes,
                      paddedSource.data() + padded.stride,
                      rowBytes),
    ...
    expect(copyBgraFramePixels(tightDestination.data(), 16, tightWithTrailingBytes),
    ...
    const std::uint32_t aboveIntMax = static_cast<std::uint32_t>(INT_MAX) + 1u;
    ...
    expect(!validateBgraFrameLayout(tooWide, nullptr, nullptr),
           "width that cannot be represented by IMAGE is rejected before allocation");
    ...
    expect(!validateBgraFrameLayout(tooTall, nullptr, nullptr),
           "height that cannot be represented by IMAGE is rejected before allocation");

    const BgraFrameView imageByteCountOverflowsInt = {
        &dummy, std::numeric_limits<std::uint32_t>::max(), 32768, 16384, 131072
    };
    expect(!validateBgraFrameLayout(imageByteCountOverflowsInt, nullptr, nullptr),
           "frame byte count that would overflow IMAGE internals is rejected");
