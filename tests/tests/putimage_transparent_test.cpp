...
int main() {
    ...
    for (const auto& res : resolutions) {
        std::cout << "\nTesting resolution: " << res.width << "x" << res.height << std::endl;
          // 生成测试图像
        auto srcImg = ImageGenerator::createCheckerboardImage(res.width, res.height);        auto bgImg = ImageGenerator::createSolidImage(res.width, res.height, static_cast<color_t>(WHITE));
          // 先绘制背景
        putimage(0, 0, bgImg);
          // 测试不同透明色的性能
        std::vector<color_t> transparentColors = {
            static_cast<color_t>(BLACK),
            static_cast<color_t>(WHITE), 
            static_cast<color_t>(RED),
            static_cast<color_t>(GREEN),
            static_cast<color_t>(BLUE),
            static_cast<color_t>(MAGENTA)
        };
        
        for (color_t transColor : transparentColors) {            std::cout << "  Transparent color: 0x" << std::hex << transColor << std::dec << std::endl;
            
            PerformanceTimer timer;
            const int iterations = 100;
            
            timer.start();            for (int i = 0; i < iterations; i++) {
                putimage_transparent(nullptr, srcImg, 0, 0, transColor, 0, 0, res.width, res.height);
            }
            timer.stop();
        ...
        for (float factor : sizeFactors) {
            ...
            timer.start();            for (int i = 0; i < iterations; i++) {
                int srcX = (i * 5) % (res.width - srcWidth);
                int srcY = (i * 5) % (res.height - srcHeight);
                putimage_transparent(nullptr, srcImg, 0, 0, static_cast<color_t>(BLACK), srcX, srcY, srcWidth, srcHeight);
            }
            timer.stop();
        ...
        timer.start();        for (int i = 0; i < iterations; i++) {
            ...
            putimage_transparent(nullptr, srcImg, destX, destY, static_cast<color_t>(BLACK), 0, 0, res.width / 2, res.height / 2);
        }
        timer.stop();
        ...
        for (int i = 0; i < iterations; i++) {
            putimage_transparent(nullptr, solidImg, 0, 0, static_cast<color_t>(BLACK), 0, 0, res.width, res.height);
        }timer.stop();
        ...
        timer.reset();        timer.start();
        for (int i = 0; i < iterations; i++) {
            putimage_transparent(nullptr, sparseImg, 0, 0, static_cast<color_t>(BLACK), 0, 0, res.width, res.height);
        }timer.stop();
