...
void testFunction(const std::string& testName, std::function<void()> testFunc, int iterations = 100) {
    ...
    std::cout << "    " << std::setw(35) << std::left << testName 
              << " - Avg: " << std::setw(8) << std::fixed << std::setprecision(3) << avgTime << " ms"
              << " | FPS: " << std::setw(8) << std::fixed << std::setprecision(1) << fps
    ...
    for (const auto& res : resolutions) {
        ...
        for (int alpha : alphaValues) {
            ...
            testFunction(testName, [&]() {
                ege::putimage_alphablend(nullptr, srcImg, 0, 0, (unsigned char)alpha, ALPHATYPE_STRAIGHT);
            });
        }
        ...
        // 版本1：基础版本 - 只有位置
        testFunction("版本1: 只有位置参数", [&]() {
            ege::putimage_alphablend(nullptr, srcImg, 10, 10, testAlpha, ALPHATYPE_STRAIGHT);
        });
        ...
        testFunction("GDI+ 无缩放", [&]() {
            ege::putimage_alphablend(nullptr, srcImg, 10, 10, res.width/2, res.height/2, 
                                   testAlpha, 0, 0, res.width/2, res.height/2, false, ALPHATYPE_STRAIGHT);
        });
        
        // 缩放 - GDI+缩放
        testFunction("GDI+ 2x缩放", [&]() {
            ege::putimage_alphablend(nullptr, smallSrcImg, 10, 10, res.width/2, res.height/2,
                                   testAlpha, 0, 0, res.width/4, res.height/4, false, ALPHATYPE_STRAIGHT);
        });
        
        // 平滑缩放
        testFunction("GDI+ 2x缩放+平滑", [&]() {
            ege::putimage_alphablend(nullptr, smallSrcImg, 10, 10, res.width/2, res.height/2,
                                   testAlpha, 0, 0, res.width/4, res.height/4, true, ALPHATYPE_STRAIGHT);
        });
        
        // 预乘Alpha + 平滑（强制使用GDI+）
        testFunction("GDI+ 预乘Alpha+平滑", [&]() {
            ege::putimage_alphablend(nullptr, srcImg, 10, 10, res.width/2, res.height/2,
                                   testAlpha, 0, 0, res.width/2, res.height/2, true, ALPHATYPE_PREMULTIPLIED);
        });
