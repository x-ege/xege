...
void testFunction(const std::string& testName, std::function<void()> testFunc, int iterations = 100) {
    ...
    std::cout << "    " << std::setw(35) << std::left << testName << " - Avg: " << std::setw(8) << std::fixed << std::setprecision(3)
              << avgTime << " ms"
              << " | FPS: " << std::setw(8) << std::fixed << std::setprecision(1) << fps << " | Ops/sec: " << std::setw(10) << std::fixed
              << std::setprecision(1) << (1000.0 / avgTime) << std::endl;
}
...
int main() {
    ...
    std::vector<ImageResolution> resolutions = {
        { 800, 600, "测试分辨率" }, { 1280, 720, "720p分辨率" }, { 1920, 1080, "全高清分辨率" },
        { 2560, 1440, "2K分辨率" }, { 3840, 2160, "4K分辨率" },
    }; // 使用中等分辨率专注于性能差异
