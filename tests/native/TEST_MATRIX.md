...
| Camera samples | `camera_base`, `camera_wave`; `EGE_ENABLE_CAMERA_CAPTURE=ON` requires initialized `3rdparty/ccap` | Compile/link in the `demos` target; live camera startup is an explicit hardware test so ordinary CTest does not access devices or show permission UI |
...
```sh
cmake -S . -B build/mac-tests -DEGE_DEFAULT_BACKEND=COREGRAPHICS -DEGE_BUILD_TEST=ON -DEGE_BUILD_DEMO=ON -DEGE_ENABLE_WINDOW_TESTS=OFF
