...
void ImageGenerator::fillImagePattern(PIMAGE img, ImageType type, color_t baseColor) {
    ...
    switch (type) {
        ...
        case GRADIENT: {
            color_t endColor = EGERGB(255 - EGEGET_R(baseColor), 
                                     255 - EGEGET_G(baseColor), 
                                     255 - EGEGET_B(baseColor));
            settarget(nullptr);
            delimage(img);
...
color_t ImageGenerator::randomColor() {
    return EGERGB(rand() % 256, rand() % 256, rand() % 256);
}
...
std::vector<PIMAGE> ImageGenerator::createTestImageSet(int width, int height) {
    ...
    images.push_back(createGradientImage(width, height, RED, BLUE, false));    images.push_back(createNoiseImage(width, height));
...
void ImageGenerator::printImageInfo(PCIMAGE img, const std::string& name) {
    ...
    std::cout << name << ": " << getwidth(img) << "x" << getheight(img) 
              << " (" << (getwidth(img) * getheight(img) * 4 / 1024) << " KB)" << std::endl;
}
